...
- **Only the most recently modified source file per (competition, season) is trusted:** `_source_file_modified_at` (the cloud storage last-modified timestamp, captured at ingestion) is compared across every file sharing a competition/season, and rows from any superseded file are excluded from `total_sheet_partners`. This is what lets a corrected re-upload replace an earlier file's data rather than being appended alongside it - the superseded file's rows remain visible in `total_sheet_partners_raw` as history, but are excluded from the table this stage's downstream consumers actually read
...
- Metadata: `_source_tab`, `_sheet_row_number` (the true Excel row, captured during parsing rather than reconstructed from explode position), `_source_file` (URL-decoded), `_source_file_modified_at` (cloud storage last-modified time, captured at ingestion and used downstream to determine the current file per competition/season), `_ingested_at`, `_season_from_filename`
...
- **Only the most recently modified source file per (competition, season) is trusted**, computed before any other logic in this transform: `_source_file_modified_at` is compared across every file sharing a competition/season (extracted here via `COMPETITION_PATTERN` and carried in from `_season_from_filename`), and rows from any superseded file are excluded via `dense_rank`. `dense_rank`, not `row_number`, is required, since every row from one file shares the same `_source_file_modified_at` value and `dense_rank` correctly keeps all of the winning file's rows across every matching tab, where `row_number` would arbitrarily cut it off partway through. This is what lets a corrected re-upload for a season already in the table replace the superseded file's rows rather than silently double-counting every invoice in that season
...
- Column trim: drops 22 columns (lineage detail, original-currency and CHF blocks, rate columns) down to 13 - the business columns plus `_source_file`, `_ingested_at`, `_season_from_filename`. EUR figures only.
